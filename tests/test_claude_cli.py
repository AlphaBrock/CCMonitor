...
from src.integrations import claude_cli
...
class TestDiscoverCliPath(unittest.TestCase):
    ...
    @patch('src.integrations.claude_cli.shutil.which')
    ...
    @patch('src.integrations.claude_cli.shutil.which', return_value=None)
    ...
    def test_last_resort_returns_default_when_nothing_found(self, _mock_which):
        ...
        with TemporaryDirectory() as tmp:
            home = Path(tmp)
            with patch('src.integrations.claude_cli.Path.home', return_value=home), \
...
class TestCliVersion(unittest.TestCase):
    ...
    @patch('src.integrations.claude_cli.subprocess.run')
...
class TestFindInstallations(unittest.TestCase):
    ...
    @patch('src.integrations.claude_cli.cli_version', return_value='')
    @patch('src.integrations.claude_cli.CLAUDE_CLI_PATH')
    @patch('src.integrations.claude_cli._EXTENSION_DIRS', [])
    ...
    @patch('src.integrations.claude_cli.cli_version', return_value='2.1.69')
    @patch('src.integrations.claude_cli.CLAUDE_CLI_PATH')
    @patch('src.integrations.claude_cli._EXTENSION_DIRS', [])
    ...
    def test_vscode_extension(self, mock_cli_path, _mock_version):
        ...
        with TemporaryDirectory() as tmp:
            ...
            with patch('src.integrations.claude_cli._EXTENSION_DIRS', [('VS Code', ext_dir)]):
                result = find_installations()
        self.assertEqual(len(result), 1)
    ...
    def test_nonexistent_extension_dir_skipped(self, mock_cli_path, _mock_version):
        ...
        mock_cli_path.is_file.return_value = False
        with patch('src.integrations.claude_cli._EXTENSION_DIRS', [('VS Code', Path('/nonexistent'))]):
            result = find_installations()
        self.assertEqual(result, [])
